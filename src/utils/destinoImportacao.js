...
export function avaliarDestinoImportacao(destino = {}) {
  ...
  const temProvaDeclaracao = Boolean(destino.importFormato || destino.documentoFonte || hasWorkingData({ ...destino, contribuinte: null }));
  return {
    ...
    temImportacaoAnterior: patrimoniais.some(campo => (destino[campo] || []).some(item => ['importacao', 'origem_legacy'].includes(item.origem))) || ((destino.origemAnoAtual ?? destino.origem) === 'importacao' && temProvaDeclaracao),
  };
}
