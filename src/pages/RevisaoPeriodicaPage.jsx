...
const ROTULOS_COMPARACAO = {
  declarado: 'Valor declarado',
  atual: 'Situação atual',
  ajustes: 'Alterações locais e retificadora',
};

const rotuloCampo = (campo) => ROTULOS_COMPARACAO[campo]
  || String(campo).replace(/([a-z])([A-Z])/g, '$1 $2').replace(/^./, letra => letra.toUpperCase());

function ComparacaoValor({ valor }) {
  if (valor == null || valor === '') return <span className="acomp-comparacao-vazio">Não informado</span>;
  if (Array.isArray(valor)) {
    if (valor.length === 0) return <span className="acomp-comparacao-vazio">Nenhum registro</span>;
    return (
      <ol className="acomp-comparacao-lista">
        {valor.map((item, indice) => <li key={item?.id || indice}><ComparacaoValor valor={item} /></li>)}
      </ol>
    );
  }
  if (typeof valor === 'object') {
    return (
      <dl className="acomp-comparacao-campos">
        {Object.entries(valor).map(([campo, conteudo]) => (
          <div key={campo}>
            <dt>{rotuloCampo(campo)}</dt>
            <dd><ComparacaoValor valor={conteudo} /></dd>
          </div>
        ))}
      </dl>
    );
...
function ComparacaoProveniencia({ comparacao }) {
  return (
    <div className="acomp-comparacao" aria-label="Comparação de proveniência">
      <div className="acomp-comparacao-grid">
        {Object.entries(comparacao).map(([campo, valor]) => (
          <section className="acomp-comparacao-card" key={campo}>
            <h4>{rotuloCampo(campo)}</h4>
            <ComparacaoValor valor={valor} />
          </section>
        ))}
      </div>
      <details className="acomp-comparacao-tecnico">
        <summary>Ver estrutura técnica completa</summary>
        <pre>{JSON.stringify(comparacao, null, 2)}</pre>
      </details>
    </div>
  );
}
...
export default function RevisaoPeriodicaPage({ onNavigate }) {
  ...
  return <>
    <div className="page-header"><h2>Revisão e pendências</h2><div className="page-header-actions"><button type="button" className="btn btn-secondary" onClick={exportar}>Exportar revisão e fichas</button><button type="button" className="btn btn-secondary" onClick={() => window.print()}>Imprimir</button></div></div>
    <div className="page-body acomp-page">
      ...
      <TabelaRedimensionavel persistKey="revisao-pendencias"><table><thead><tr><th>Tipo</th><th>Origem / pendência</th><th>Pareceres registrados</th><th>Ações</th></tr></thead><tbody>{pendencias.map(p => <tr key={p.id}><td>{p.tipo}</td><td>{p.descricao}</td><td>{p.pareceres.map(r => <p key={r.id}>{formatDateTime(r.criadoEm)} - {r.responsavel}: {r.decisao}</p>)}</td><td><div className="acomp-acoes"><button type="button" className="btn btn-sm btn-secondary" onClick={() => { setSelecionada(p); setDecisao(''); setErro(''); }}>Registrar parecer</button><button type="button" className="btn btn-sm btn-secondary" onClick={() => { if (p.ref && p.ref.ano !== state.anoCalendario) dispatch({ type: 'SWITCH_ANO', payload: p.ref.ano }); onNavigate?.(p.acao); }}>Abrir ficha</button></div></td></tr>)}{!pendencias.length && <tr><td colSpan="4">Nenhuma pendência detectada pelas regras automatizadas. Ainda é necessária a revisão humana do checklist.</td></tr>}</tbody></table></TabelaRedimensionavel>
    ...
    <Modal open={!!selecionada} onClose={() => { if (!ocupado) setSelecionada(null); }}><form onSubmit={parecer}><div className="modal-header"><h3>Revisão do registro</h3><button type="button" className="btn btn-secondary" onClick={() => setSelecionada(null)}>Fechar</button></div><div className="modal-body acomp-form"><p>{selecionada?.descricao}</p>{selecionada?.comparacao && <ComparacaoProveniencia comparacao={selecionada.comparacao} />}{erro && <p role="alert">{erro}</p>}<label className="form-group">Responsável<input required value={responsavel} onChange={e => setResponsavel(e.target.value)} /></label><label className="form-group">Decisão / providência<textarea required value={decisao} onChange={e => setDecisao(e.target.value)} /></label><p>Registrar parecer não altera valores declarados nem elimina pendências que ainda tenham causa presente.</p></div><div className="modal-footer"><button type="submit" className="btn btn-primary" disabled={ocupado}>Salvar parecer</button></div></form></Modal>
  </>;
}
