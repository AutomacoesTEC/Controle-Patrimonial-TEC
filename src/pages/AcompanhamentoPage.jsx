import { CATEGORIAS_FLUXO } from '../store/classificacoesFinanceiras';
...
export default function AcompanhamentoPage({ contextoInicial, onVoltar } = {}) {
  const { state, dispatchPersistido, addToast } = useData();
  const a = acompanhamentoDo(state);
  const referencias = referenciasFiscais(state);
  const refOrigem = contextoInicial?.referenciaOrigem;
  const origem = refOrigem && referencias.find(r => chaveReferencia(r.ref) === chaveReferencia(refOrigem));
  const vinculadasOrigem = origem ? a.operacoes.filter(o => (o.vinculos || []).some(v => chaveReferencia({ ...v.ref, movimentacaoId: undefined }) === chaveReferencia(origem.ref))) : [];
  const [aba, setAba] = useState(contextoInicial?.aba || 'caixa');
  const [mes, setMes] = useState(contextoInicial?.mes || `${state.anoCalendario || new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, '0')}`);
  const [pessoa, setPessoa] = useState(contextoInicial?.pessoa || 'todos');
  const [operacao, setOperacao] = useState(contextoInicial?.operacao || '');
  ...
  // `opcional` (P02): o select pode ser salvo em branco — é o caso do
  // "Classificar depois" da classificação de fontes/aplicações, que o domínio
  // aceita vazio (ver acompanhamento.js). Os demais selects seguem required.
  const campo = (nome, rotulo, tipo = 'text', opcoes, aoMudar, opcional = false) => <label className="form-group" key={nome}><span>{rotulo}</span>{tipo === 'select' ? <select className="form-control" aria-label={rotulo} required={!opcional} value={form[nome] || ''} onChange={e => setForm(f => ({ ...f, [nome]: e.target.value, ...(aoMudar ? aoMudar(e.target.value) : {}) }))}>{!opcional && <option value="">Selecione</option>}{opcoes.map(([v, t]) => <option key={v} value={v}>{t}</option>)}</select> : <input className="form-control" aria-label={rotulo} required type={tipo} step={tipo === 'number' ? '0.01' : undefined} value={form[nome] ?? ''} onChange={e => setForm(f => ({ ...f, [nome]: e.target.value }))} />}</label>;
  ...
  return <>
    ...
    <div className="page-body acomp-page">
      ...
      {aba === 'caixa' && <>
        <section className="card acomp-card">
          ...
          <dl className="acomp-saldos">{[['Saldo inicial', resumo.inicial], ['Aberturas no período', resumo.aberturas], ['Entradas', resumo.entradas], ['Saídas', resumo.saidas], ['Transferências líquidas no recorte', resumo.transferencias]].map(([rotulo, valor]) => <div key={rotulo}><dt>{rotulo}</dt><dd>{dinheiro(valor)}</dd></div>)}</dl>
          <div className="acomp-saldo-final"><span>Saldo final registrado</span><strong>{resumo.contas ? dinheiro(resumo.final) : 'Sem contas disponíveis cadastradas'}</strong></div>
          <p className="acomp-nota">Fluxo previsto ainda sem baixa: {dinheiro(resumo.projetado)}. Somente contas marcadas como disponíveis compõem este saldo; sua comprovação depende dos extratos.</p>
        </section>
        ...
        <Tabela id="acomp-movimentos" colunas={['Data financeira', 'Conta / destino', 'Descrição / contraparte', 'Operação', 'Tipo', 'Valor', 'Ações']} linhas={a.lancamentos.filter(l => l.data >= de && l.data <= ate && (pessoa === 'todos' || contasVisiveis.some(c => c.id === l.contaId || c.id === l.destinoId))).map(l => [formatDate(l.data), `${nomeConta(l.contaId)}${l.destinoId ? ` → ${nomeConta(l.destinoId)}` : ''}`, `${l.descricao} / ${l.contraparte}`, nomeOperacao(l.operacaoId), l.canceladoEm ? `Cancelado: ${l.motivoCancelamento}` : l.tipo, dinheiro(l.valor), !l.canceladoEm && botoes([...(l.tipo !== 'transferencia' ? [['Classificar', () => abrir('classificarLancamento', { id: l.id, tipo: l.tipo, categoriaFluxo: l.categoriaFluxo || '' })]] : []), ['Cancelar baixa', () => abrir('cancelarLancamento', { id: l.id })]])])} />
      </>}
      {aba === 'operacoes' && <>
        {refOrigem && <section className="card acomp-card"><h3>Bem indicado no demonstrativo</h3><p>{origem?.rotulo || 'Registro não encontrado. Retorne ao demonstrativo para atualizar a referência.'}</p>
          {origem && (vinculadasOrigem.length ? <><p>Vínculo registrado. Selecione a operação para conferir parcelas, documentos e baixas.</p>{botoes(vinculadasOrigem.map(o => ['Abrir operação: ' + o.descricao, () => { setPessoa(o.pessoa); setOperacao(o.id); }]))}</> : <><p>Selecione uma operação existente ou cadastre a venda em Nova operação. Depois confirme o vínculo com este registro.</p><button type="button" className="btn btn-primary" onClick={() => abrir('vincular', { referenciaFiscal: chaveReferencia(origem.ref), operacaoId: operacao })}>Vincular este registro</button></>)}
        </section>}
        ...
        <label className="form-group">Operação em foco<select className="form-control" aria-label="Operação em foco" value={operacao} onChange={e => setOperacao(e.target.value)}><option value="">Todas as operações</option>{ops.map(o => <option key={o.id} value={o.id}>{o.descricao}</option>)}</select></label>
        <Tabela id="acomp-operacoes" colunas={['ID / descrição', 'Titularidade', 'Fato econômico', 'Ano fiscal', 'Registrado em', 'Tipo', 'Ações']} linhas={ops.filter(o => !operacao || o.id === operacao).map(o => [`${o.id} ; ${o.descricao}`, nomePessoa(o.pessoa), formatDate(o.dataEconomica), o.anoFiscal, formatDateTime(o.criadoEm), o.tipo, botoes([['Editar operação', () => abrir('editarOperacao', { ...o, precoContrato: o.precoContrato == null ? '' : o.precoContrato / 100, custoBaixado: o.custoBaixado == null ? '' : o.custoBaixado / 100, despesasVenda: o.despesasVenda == null ? '' : o.despesasVenda / 100 })]])])} />
        ...
        <Tabela id="acomp-documentos" colunas={['Operação', 'Documento', 'Referência local', 'Registrado em']} linhas={a.documentos.filter(d => !operacao || d.operacaoId === operacao).map(d => [nomeOperacao(d.operacaoId), d.descricao, d.referencia, formatDateTime(d.criadoEm)])} />
      </>}
      {aba === 'conciliacao' && <>
      ...
      {aba === 'fechamento' && <>
        ...
        <Tabela id="acomp-fechamentos" colunas={['Mês', 'Versão', 'Situação', 'Responsável', 'Data', 'Parecer / motivo']} linhas={a.fechamentos.map(f => [f.mes, f.versao, f.status, f.responsavel, formatDateTime(f.criadoEm), f.observacoes || f.motivo])} />
      </>}
      {aba === 'economico' && <>
    ...
    <Modal open={modal != null} onClose={() => { if (!ocupado) setModal(null); }}><form onSubmit={salvar}><div className="modal-header"><h3>{TITULOS[modal]}</h3><button type="button" className="btn btn-secondary" disabled={ocupado} onClick={() => setModal(null)}>Fechar</button></div><div className="modal-body acomp-form">
      ...
      {modal === 'lancamento' && <>{campo('tipo', 'Tipo de baixa', 'select', [['entrada', 'Entrada'], ['saida', 'Saída'], ['transferencia', 'Transferência própria']], valor => ({ ...ajusteDoTipoDeBaixa('lancamento', valor), categoriaFluxo: '' }))}{camposConta()}{form.tipo === 'transferencia' && camposConta('destinoId', 'Conta de destino')}{campo('data', 'Data efetiva de pagamento / recebimento', 'date')}{campo('valor', 'Valor efetivo (R$)', 'number')}{campo('descricao', 'Descrição da baixa')}{form.tipo !== 'transferencia' && <>{campo('contraparte', 'Contraparte (pagador / recebedor)')}{campo('categoriaFluxo', 'Classificação para fontes e aplicações', 'select', [['', 'Classificar depois'], ...CATEGORIAS_FLUXO.filter(c => c[2] === form.tipo).map(c => [c[0], c[1]])], undefined, true)}<label className="form-group">Operação vinculada (opcional)<select className="form-control" value={form.operacaoId || ''} onChange={e => setForm(f => ({ ...f, operacaoId: e.target.value }))}><option value="">Sem vínculo - ficará pendente</option>{a.operacoes.map(o => <option key={o.id} value={o.id}>{o.descricao}</option>)}</select></label></>}{form.parcelaId && <p>Baixa integral da parcela selecionada. O total e a operação serão validados ao salvar.</p>}</>}
      {modal === 'classificarLancamento' && campo('categoriaFluxo', 'Classificação para fontes e aplicações', 'select', [['', 'Classificar depois'], ...CATEGORIAS_FLUXO.filter(c => c[2] === form.tipo).map(c => [c[0], c[1]])], undefined, true)}
